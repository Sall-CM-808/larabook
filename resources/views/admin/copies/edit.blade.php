    ...
    <div class="py-6">
        <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="bg-slate-800 rounded-2xl p-6 border border-white/10 shadow-lg shadow-black/10">
                <h2 class="text-xl font-bold text-white mb-6">Modifier l'exemplaire : {{ $copy->code_barre }}</h2>
                ...
                <form method="POST" action="{{ route('admin.copies.update', $copy) }}" class="space-y-5">
                    @csrf @method('PUT')
                    <div>
                        ...
                        <select name="book_id" required class="w-full bg-slate-900 border border-white/10 rounded-lg px-4 py-2.5 text-sm text-slate-300 focus:border-indigo-500/50 focus:outline-none">
                            @foreach($books as $book)
                                <option value="{{ $book->id }}" {{ old('book_id', $copy->book_id) == $book->id ? 'selected' : '' }}>{{ $book->titre }} - {{ $book->auteur }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                        <label class="block text-sm text-slate-400 mb-1">Code-barres *</label>
                        <input type="text" name="code_barre" value="{{ old('code_barre', $copy->code_barre) }}" required class="w-full bg-slate-900 border border-white/10 rounded-lg px-4 py-2.5 text-sm text-slate-300 focus:border-indigo-500/50 focus:outline-none">
                    </div>
                    <div>
                        ...
                        <select name="etat" required class="w-full bg-slate-900 border border-white/10 rounded-lg px-4 py-2.5 text-sm text-slate-300 focus:border-indigo-500/50 focus:outline-none">
                            @foreach(['disponible', 'emprunte', 'perdu', 'maintenance'] as $etat)
                                <option value="{{ $etat }}" {{ old('etat', $copy->etat) === $etat ? 'selected' : '' }}>{{ ucfirst($etat) }}</option>
                            @endforeach
                        </select>
                    </div>
